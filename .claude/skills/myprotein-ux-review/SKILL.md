...
values, border radii, type specs, and component specs from the Myprotein design system,
extracted from live production Figma files. This is your ground truth for every value
mentioned below — if anything here ever looks like it disagrees with that file, the reference
file wins; it's the more current, verified source.
...
- **Colours:** Inspect `background-color` and `color` on the primary CTA, headings,
  links, badges, radio/selection states, and promo banners. Compare the computed hex to the
  token table in `design-tokens.md`. Flag any hex not in that table — it's a hardcoded
  override. Also flag accent colours used outside their defined purpose (e.g. mint
  `#94E5B5` used as a fill instead of a selection ring).

- **Spacing:** Inspect padding and margin on product cards, buttons, and section gaps
  against the spacing scale in `design-tokens.md`. Flag anything off-scale.

- **Border radius:** Check cards, buttons, chips, and selectors against the radius values
  and component specs in `design-tokens.md`. Flag deviations, especially loss of the
  pill-shape signature on buttons/tabs.

- **Typography:** Verify Figtree is the computed font family (not a fallback). Check font
  sizes against the type scale in `design-tokens.md`. Flag title case headings, American
  spellings, or emoji.

- **Component-specific signatures:** `design-tokens.md` documents brand-specific details for
  key components (e.g. the Add to Basket button's sticky mobile behaviour, the radio-tab
  selection ring, the nav pill active indicator). Check these against the live page — they're
  easy to miss because they're not simple colour/spacing checks.
...
See `references/design-tokens.md` for the full token and component reference and the "What to
always flag" quick-reference list at the bottom, including pre-assigned severity levels. Note
that file is a work in progress — it flags its own lower-confidence entries, so treat those
more as leads than hard rules.
