...
### Unmapped colours (no Tailwind class as of this capture — flag with ⚠️)
- #DD592B (orange/coral — appears in `.search` CSS only). Note: as of the July 2026
  `myprotein-ux-review` design-token capture, this is a confirmed live design token
  (`accent.burntOrange`, used for active/selected state indicators e.g. the active nav pill
  underline) — it's not a stray/unintentional colour. It simply had no matching Tailwind
  class at the time this config was captured. Still flag it ⚠️ in a handover, but frame it as
  "needs a Tailwind class added" rather than "not a real design token."
