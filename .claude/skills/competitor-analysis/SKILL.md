...
Use the Browser pane tools (`mcp__Claude_Browser__*`) for navigation, screenshots, and reading
page content. `preview_start` with a `url` opens an external site directly in its own tab, and
`navigate`, `computer` (`screenshot` action), `read_page`, and `get_page_text` all work
straightforwardly on competitor domains — no blocking issue, no one-time setup required. This
is the default path.

Note: the `computer` screenshot action returns an image for visual inspection but does not
currently save a file to disk. Use it to *look at* the page and inform your written findings,
but don't assume screenshot files land in the repo automatically — flag them in the
screenshots checklist below as not-yet-saved so the user knows to capture them manually, the
same as the AppleScript fallback path.

**Fallback — only if the Browser pane tools aren't available in your environment** and you're
limited to `mcp__Claude_in_Chrome__navigate`: that tool is blocked on most external domains, so
use Chrome directly via AppleScript instead.

One-time setup: in Chrome, enable **View → Developer → Allow JavaScript from Apple Events**
(persists across sessions). Then:
...
On this fallback path, screenshots aren't available on those Chrome tabs — note which
screenshots are needed in the checklist and let the user capture them manually.
