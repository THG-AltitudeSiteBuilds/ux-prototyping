...
This reference is a point-in-time capture of the engineering team's actual Tailwind config —
it isn't regenerated automatically, so it can drift from the real codebase over time. Check
the "Notes" section at the bottom of the file for when it was last captured, and if it's been
a while, say so in your output rather than presenting every mapping as unquestionably current.
...
0. **Load the `figma-use` skill first.** It's a mandatory prerequisite before any `use_figma`
   call — skipping it is a common source of hard-to-debug failures. Do this before step 1
   below, not after.
